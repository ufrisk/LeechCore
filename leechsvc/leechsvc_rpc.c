...
VOID LeechSvcRpc_WriteInfoEventLog(_In_z_ _Printf_format_string_ char const* const _Format, ...)
{
    ...
    DWORD dwSize;
    LPSTR szArgs[2] = { "LeechService:", "" };
    va_start(argptr, _Format);
    dwSize = (DWORD)vsnprintf(szBuffer, 0x1000 - 1, _Format, argptr);
    va_end(argptr);
    if(!dwSize) { return; }
    hEventInfoLog = RegisterEventSourceA(NULL, "LeechSvc");
...
RPC_STATUS CALLBACK LeechSvcRpc_SecurityCallback(RPC_IF_HANDLE InterfaceUuid, void *Context)
{
    ...
    for(i = 0; i < cEntries; i++) {
        ...
        if(!wcscmp(wszAdministrators, pLocalGroupInfo->lgrui0_name)) {
            ...
            LeechSvcRpc_WriteInfoEventLog("Remote User: '%S' successfully authenticated towards the RPC service.\n", wszUser);
            return RPC_S_OK;
        }
    }
    NetApiBufferFree(pbLocalGroups);
    LeechSvcRpc_WriteInfoEventLog("Remote User: '%S' failed to authenticate towards the RPC service - not in Administrators group?.\n", wszUser);
...
RPC_STATUS RpcStart(_In_ BOOL fInsecure, _In_ BOOL fSvc)
{
    ...
    //status = RpcServerRegisterIf(LeechRpc_v1_0_s_ifspec, 0, 0);
...
#pragma warning(suppress: 6102)
    ...
    // Set security mode.
    if(fInsecure) {
        printf("WARNING! Starting LeechSvc in INSECURE mode - WARNING! any user may\n     connect unauthenticated to service unless properly firewalled!\n");
    } else {
        ...
        printf("Starting LeechSvc with kerberos SPN: ' %s '\n    (specify the SPN value in client connection string).\n    (ensure that port tcp/28473 is open in firewall).\n", szSPN);
        if(fSvc) {
            LeechSvcRpc_WriteInfoEventLog("LeechSvc started with kerberos SPN: %s\n", szSPN);
        }
        RpcStringFreeA(&szSPN);
    }
